 ...
 *   `resolveChannelsForUser` seeds the channel set from the MANDATORY channels
 *   and returns mandatory-only when a user disables a type, so an in-app row was
 *   written for every recipient of every event no matter what the user had
 *   configured — the whole preference system was decorative for the bell.
 *
 *   IN_APP is now `mandatory: false` everywhere EXCEPT the safety/compliance
 *   allowlist below, which must remain unsilenceable:
 *
 *     warning.issued, warning.issued.escalation      (advertências)
 *     ppe.signature_required, ppe.signature_failed   (assinatura obrigatória de EPI)
 *     ppe.ca_expiry                                  (CA vencido — NR-6)
 *     fispq.expiring                                 (FISPQ/FDS — segurança química)
 *     medical_exam.expiring, medical_exam.return_due (ASO — medicina do trabalho)
 *     secullum.signature.ready,
 *     secullum.signature.rejected                    (assinatura do cartão-ponto)
 *     vacation.concessive_expired,
 *     vacation.concessive_expiring                   (CLT art. 137 — férias em dobro)
 *
 *   `enabled` and `defaultOn` are unchanged, so users who never touched their
 *   preferences keep receiving exactly what they receive today; only an explicit
 *   opt-out now actually works.
 *
 * THROTTLING THE TWO NOISIEST FAMILIES
 *   The 34 `task.field.*` and 44 `service_order.*` configs carry
 *   `deduplicationWindow: 60` (minutes) and `maxFrequencyPerDay: 5`.
 *   NEVER write 0 to either: the enforcement gates are truthiness checks
 *   (notification-configuration.service.ts), so 0 silently DISABLES the check —
 *   which is exactly what task.field.bankSlips did before this change.
 *   Note the dedup key is (configKey, entityId) and is GLOBAL — one suppression
 *   covers everyone, including someone who never saw the first notification.
 *   maxFrequencyPerDay counts PER RECIPIENT (groupBy userId) but blocks the
...
const CONFIGS: ConfigDef[] = [
  ...
  // ─── NFS-e do aerografista (prestador MEI, Sistema Nacional) ─────────────────
  {
    ...
    name: "NFS-e de Aerografia Emitida",
    notificationType: "SYSTEM",
  ...
  {
    key: "airbrushing.nfse.rejected",
    name: "NFS-e de Aerografia Rejeitada",
    notificationType: "SYSTEM",
    eventType: "airbrushing.nfse.rejected",
    description:
      "A emissão da NFS-e do aerografista falhou. Enquanto não for corrigida, a aerografia fica concluída sem nota fiscal do prestador.",
    enabled: true,
    importance: "URGENT",
    workHoursOnly: false,
    batchingEnabled: false,
  ...
  {
    key: "airbrushing.nfse.certificate_expiring",
    name: "Certificado Digital de Aerografista Vencendo",
    notificationType: "SYSTEM",
    eventType: "airbrushing.nfse.certificate_expiring",
    description:
      "O certificado A1 de um aerografista está perto de vencer. Sem certificado válido a emissão automática para por completo.",
    enabled: true,
    importance: "HIGH",
    workHoursOnly: false,
    batchingEnabled: false,
  ...
  {
    key: "task.field.layouts",
    name: "Artes da Tarefa Atualizadas",
    notificationType: "PRODUCTION",
    eventType: "task.field.layouts",
    ...
    metadata: {
      ...
      isFileArray: true,
      registry: "seed-notification-configs",
      trigger: "ver dispatchByConfiguration(\"task.field.layouts\") no código (emissor não anotado)",
      targeted: false,
    },
  },
  {
  ...
  {
    key: "task.field.implement.rearDoorBarCount",
    name: "Varões da Porta Traseira Alterados",
    notificationType: "PRODUCTION",
    eventType: "task.field.implement.rearDoorBarCount",
    description: "Quantidade de varões da porta traseira do implemento alterada (rastreador de campos da tarefa e PUT /implements).",
    enabled: true,
    importance: "NORMAL",
    workHoursOnly: true,
    batchingEnabled: false,
    maxFrequencyPerDay: 5,
    deduplicationWindow: 60,
    sectors: ["ADMIN", "DESIGNER", "PRODUCTION"],
    channels: {
      IN_APP: { enabled: true, mandatory: false, defaultOn: true },
      PUSH: { enabled: true, mandatory: false, defaultOn: false },
      EMAIL: { enabled: false, mandatory: false, defaultOn: false },
    ...
    templates: {
      inApp: {
        title: "Varões da Porta Traseira Alterados",
        body: "A tarefa \"{{taskName}}\"{{#if serialNumber}} #{{serialNumber}}{{/if}} teve a quantidade de varões da porta traseira alterada{{#if oldValue}} de \"{{oldValue}}\"{{/if}}{{#if newValue}} para \"{{newValue}}\"{{/if}}.",
      },
      push: {
        title: "Varões da Porta Traseira Alterados",
        body: "{{taskName}}{{#if serialNumber}} #{{serialNumber}}{{/if}} — varões da porta traseira: {{#if newValue}}{{newValue}}{{else}}não informada{{/if}}",
      },
      email: {
        subject: "Varões da Porta Traseira Alterados - {{taskName}}",
        body: "A tarefa teve a quantidade de varões da porta traseira alterada.\n\nTarefa: {{taskName}}\n{{#if serialNumber}}Número de Série: {{serialNumber}}\n{{/if}}{{#if oldValue}}Valor anterior: {{oldValue}}\n{{/if}}{{#if newValue}}Novo valor: {{newValue}}\n{{/if}}",
      },
      whatsapp: {
        body: "A tarefa \"{{taskName}}\"{{#if serialNumber}} #{{serialNumber}}{{/if}} teve a quantidade de varões da porta traseira alterada{{#if oldValue}} de \"{{oldValue}}\"{{/if}}{{#if newValue}} para \"{{newValue}}\"{{/if}}.",
      },
    ...
    metadata: {
      field: "implement.rearDoorBarCount",
      category: "PRODUCTION",
      registry: "seed-notification-configs",
      trigger: "task.listener.ts (task.field.changed) — TaskFieldTrackerService e ImplementService.update",
      targeted: false,
    },
  },
  {
    key: "task.field.implement.rearDoorHatchCount",
    name: "Portinholas da Porta Traseira Alteradas",
    notificationType: "PRODUCTION",
    eventType: "task.field.implement.rearDoorHatchCount",
    description: "Quantidade de portinholas da porta traseira do implemento alterada (rastreador de campos da tarefa e PUT /implements).",
    enabled: true,
    importance: "NORMAL",
    workHoursOnly: true,
    batchingEnabled: false,
    ...
    templates: {
      inApp: {
        title: "Portinholas da Porta Traseira Alteradas",
        body: "A tarefa \"{{taskName}}\"{{#if serialNumber}} #{{serialNumber}}{{/if}} teve a quantidade de portinholas da porta traseira alterada{{#if oldValue}} de \"{{oldValue}}\"{{/if}}{{#if newValue}} para \"{{newValue}}\"{{/if}}.",
      },
      push: {
        title: "Portinholas da Porta Traseira Alteradas",
        body: "{{taskName}}{{#if serialNumber}} #{{serialNumber}}{{/if}} — portinholas da porta traseira: {{#if newValue}}{{newValue}}{{else}}não informada{{/if}}",
      },
      email: {
        subject: "Portinholas da Porta Traseira Alteradas - {{taskName}}",
        body: "A tarefa teve a quantidade de portinholas da porta traseira alterada.\n\nTarefa: {{taskName}}\n{{#if serialNumber}}Número de Série: {{serialNumber}}\n{{/if}}{{#if oldValue}}Valor anterior: {{oldValue}}\n{{/if}}{{#if newValue}}Novo valor: {{newValue}}\n{{/if}}",
      },
      whatsapp: {
        body: "A tarefa \"{{taskName}}\"{{#if serialNumber}} #{{serialNumber}}{{/if}} teve a quantidade de portinholas da porta traseira alterada{{#if oldValue}} de \"{{oldValue}}\"{{/if}}{{#if newValue}} para \"{{newValue}}\"{{/if}}.",
      },
    ...
    metadata: {
      field: "implement.rearDoorHatchCount",
      category: "PRODUCTION",
      registry: "seed-notification-configs",
      trigger: "task.listener.ts (task.field.changed) — TaskFieldTrackerService e ImplementService.update",
      targeted: false,
  ...
  {
    key: "task.field.implement.rearDoorLeaves",
    name: "Folhas da Porta Traseira Alteradas",
    notificationType: "PRODUCTION",
    eventType: "task.field.implement.rearDoorLeaves",
    description: "Configuração da porta traseira do implemento (bipartida/tripartida) alterada (rastreador de campos da tarefa e PUT /implements).",
    enabled: true,
    importance: "NORMAL",
    workHoursOnly: true,
    batchingEnabled: false,
    ...
    templates: {
      inApp: {
        title: "Folhas da Porta Traseira Alteradas",
        body: "A tarefa \"{{taskName}}\"{{#if serialNumber}} #{{serialNumber}}{{/if}} teve a configuração da porta traseira (bipartida/tripartida) alterada{{#if oldValue}} de \"{{oldValue}}\"{{/if}}{{#if newValue}} para \"{{newValue}}\"{{/if}}.",
      },
      push: {
        title: "Folhas da Porta Traseira Alteradas",
        body: "{{taskName}}{{#if serialNumber}} #{{serialNumber}}{{/if}} — porta traseira: {{#if newValue}}{{newValue}}{{else}}não informada{{/if}}",
      },
      email: {
        subject: "Folhas da Porta Traseira Alteradas - {{taskName}}",
        body: "A tarefa teve a configuração da porta traseira (bipartida/tripartida) alterada.\n\nTarefa: {{taskName}}\n{{#if serialNumber}}Número de Série: {{serialNumber}}\n{{/if}}{{#if oldValue}}Valor anterior: {{oldValue}}\n{{/if}}{{#if newValue}}Novo valor: {{newValue}}\n{{/if}}",
      },
      whatsapp: {
        body: "A tarefa \"{{taskName}}\"{{#if serialNumber}} #{{serialNumber}}{{/if}} teve a configuração da porta traseira (bipartida/tripartida) alterada{{#if oldValue}} de \"{{oldValue}}\"{{/if}}{{#if newValue}} para \"{{newValue}}\"{{/if}}.",
      },
    ...
    metadata: {
      field: "implement.rearDoorLeaves",
      category: "PRODUCTION",
      registry: "seed-notification-configs",
      trigger: "task.listener.ts (task.field.changed) — TaskFieldTrackerService e ImplementService.update",
      targeted: false,
