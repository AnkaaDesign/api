 ...
 */
export function mapServiceOrderStatusToPrisma(
  status: SERVICE_ORDER_STATUS | string,
): ServiceOrderStatus {
...
export const getServiceOrderStatus = (
  serviceOrder: Pick<ServiceOrder, 'startedAt' | 'finishedAt'>,
...
export const calculateServiceOrderDuration = (
  serviceOrder: Pick<ServiceOrder, 'startedAt' | 'finishedAt'>,
): number | null => {
...
export const canStartServiceOrder = (
  serviceOrder: Pick<ServiceOrder, 'startedAt' | 'finishedAt'>,
): boolean => {
  return !serviceOrder.startedAt && !serviceOrder.finishedAt;
};

export const canFinishServiceOrder = (
  serviceOrder: Pick<ServiceOrder, 'startedAt' | 'finishedAt'>,
): boolean => {
