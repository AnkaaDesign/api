...
export const detectContactMethod = (input: string): 'email' | 'phone' | 'unknown' => {
  ...
  if (numericOnly.length >= 10 && numericOnly.length <= 15) {
    return isValidBrazilianPhoneEnhanced(trimmed) || isValidInternationalPhone(trimmed)
      ? 'phone'
      : 'unknown';
  }
...
export const validateVerificationRequest = (data: {
  ...
  return {
    isValid: errors.length === 0,
    errors,
    detectedContactMethod:
      contactValidation.type === 'unknown' ? undefined : contactValidation.type,
  };
};
...
export const isValidCNH = (cnh: string): boolean => {
  ...
  let remainder = sum % 11;
  const firstDigit = remainder >= 2 ? 11 - remainder : 0;
  ...
  remainder = sum % 11;
  const secondDigit = remainder >= 2 ? 11 - remainder : 0;
