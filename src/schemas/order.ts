...
export const orderUpdateSchema = z
  .object({
    ...
    status: z
      ...
      .optional(),
    // Payment status — settled through the dedicated cascade in OrderService.update
    // (stamps paidAt/paidById, settles or reopens parcelas, recomputes the rollup).
    paymentStatus: z
    ...
    discount: z
      ...
      .transform(val => Math.round(val * 100) / 100)
      .optional(),
    // Manual grand-total override (Valor Total). null = clear override / use computed total.
    totalOverride: z
      .number()
      .min(0, 'Valor total deve ser maior ou igual a 0')
      .transform(val => Math.round(val * 100) / 100)
