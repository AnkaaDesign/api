import { Layout, Task, User } from '../../../types';

/**
 * Event emitted when an layout status changes to APPROVED
 */
export class LayoutApprovedEvent {
  constructor(
    public readonly layout: Layout,
    public readonly task: Task | null,
    public readonly approvedBy: User,
  ) {}
}

/**
 * Event emitted when an layout status changes to REPROVED (rejected)
 */
export class LayoutReprovedEvent {
  constructor(
    public readonly layout: Layout,
    public readonly task: Task | null,
    public readonly reprovedBy: User,
    public readonly reason?: string,
  ) {}
}

/**
 * Event emitted when an layout is pending approval for too long (reminder)
 * This can be triggered by a cron job
 */
export class LayoutPendingApprovalReminderEvent {
  constructor(
    public readonly layout: Layout,
    public readonly task: Task | null,
    public readonly daysPending: number,
  ) {}
}
