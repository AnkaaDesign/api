    ...
    tasks.forEach(task => {
      ...
      if (task.generalPainting) {
        recentPaintUsageInTasks.push({
          ...
          taskSerialNumber: task.implement?.serialNumber || undefined,
  ...
  async getOpenServiceOrdersByTypes(
    ...
    const orders = await this.prisma.serviceOrder.findMany({
      ...
      select: {
        ...
        status: true,
        createdAt: true,
        task: { select: { id: true, name: true, implement: { select: { serialNumber: true } }, forecastDate: true } },
    ...
    return orders.map(o => ({
      ...
      taskSerialNumber: o.task.implement?.serialNumber,
