...
import type { FilesFolderMapping } from './files-storage.service';
...
> = {
  ...
  'AdmissionDocument.fileId': { context: null, label: 'documento de admissão' },
  'AdmissionDocument.signedFileId': {
    context: null,
    label: 'documento de admissão assinado',
  },
  'TerminationDocument.fileId': { context: null, label: 'documento de rescisão' },
  'MedicalExam.fileId': { context: null, label: 'ASO / exame médico' },
  'UserBenefit.declarationFileId': { context: null, label: 'declaração de benefício' },
  'WarningSignature.signedDocumentId': { context: null, label: 'advertência assinada' },
  'PpeDelivery.deliveryDocumentId': { context: null, label: 'documento de entrega de EPI' },
  'WorkAccidentReport.fileId': { context: null, label: 'CAT / acidente de trabalho' },
  '_FileToLeave.A': { context: null, label: 'afastamento' },
  'BankSlip.pdfFileId': { context: null, label: 'PDF de boleto' },
  'BankTransaction.rawFileId': { context: null, label: 'extrato bancário importado' },
  'FiscalDocument.rawXmlFileId': { context: null, label: 'XML de documento fiscal' },
  ...
  'SignatureEnvelope.originalFileId': {
    context: null,
    label: 'documento original de assinatura',
  },
...
export class FileReferenceService implements OnModuleInit {
  ...
  async getReferences(
    ...
    for (const { table, column } of columns) {
      ...
      const hit = await client.$queryRawUnsafe<Array<unknown>>(`${sql} LIMIT 1`, ...params);
      if (hit.length > 0) {
  ...
  async resolveCanonicalContext(
    ...
    try {
      ...
      // A file shared across contexts (the same image as both task layout and quote
      // layout, say) has no single right answer. Moving it would break the other
      // context's expectations, so we decline rather than guess.
      const distinct = [...new Set(withContext.map(r => r.context))];
      if (distinct.length > 1) {
        this.logger.debug(
          `[FileReference] ${fileId} é referenciado por múltiplos contextos ` +
            `(${distinct.join(', ')}) — mantido onde está.`,
        );
        return null;
      }

      return distinct[0] as keyof FilesFolderMapping;
    } catch (error: any) {
      this.logger.error(
