...
// Worker categories that belong on the CLT payroll (folha). Terceirizado/PJ/intern/autônomo
// are invoiced or have no vínculo and must be excluded from payroll/bonus/CLT-termination logic.
...
// Use `isBonifiable(contract)` / `BONIFIABLE_USER_WHERE` from `@utils/contract`
// (Part A: eligible = employeeType CLT && status ACTIVE). The former
// BONIFIABLE_CONTRACT_TYPES modality list has been RETIRED.
...
export enum SALARY_ADJUSTMENT_TYPE {
  ...
  REFRAME = 'REFRAME',
  OTHER = 'OTHER',
  // Bonus period reajuste. Not a user-selectable salary type — emitted by
  // applyPeriodAdjustment. These rows ARE the source of truth for the bonus
  // period adjustment (the engine sums them) and appear in the Reajustes history.
  BONUS = 'BONUS',
}

/**
 * Motivos de mudança de cargo (vínculo CLT).
 *
 * IMPORTANTE — base legal trabalhista (CLT/CF):
 * O *rebaixamento* unilateral com redução de cargo/salário é, em regra, ILEGAL:
 * viola a irredutibilidade salarial (CF art. 7º, VI) e a inalterabilidade
 * contratual lesiva (CLT art. 468). Por isso NÃO expomos "Rebaixamento" como
 * motivo. Os únicos movimentos descendentes/laterais lícitos são tipificados
 * abaixo, reutilizando os valores de enum já existentes (sem migração):
 *
 *  - PROMOTION  → Promoção (ascensão de cargo/salário). Sempre lícita.
 *  - TRANSFER   → Transferência de setor/local (CLT art. 469).
 *  - DEMOTION   → REVERSÃO: retorno do empregado de um cargo de confiança ao
 *                 cargo efetivo anterior (CLT art. 468, parágrafo único). É
 *                 lícita e NÃO configura alteração contratual lesiva, pois a
 *                 gratificação de função pode ser suprimida com o retorno.
 *                 (O valor de enum mantém o nome "DEMOTION" só por
 *                 compatibilidade de banco; o significado de negócio é Reversão.)
 *  - ADJUSTMENT → READAPTAÇÃO: realocação para função compatível por restrição
 *                 médica/INSS (CLT art. 461, §4º). Lícita; protege o empregado.
 *  - CORRECTION → REENQUADRAMENTO: reclassificação por reestruturação do plano
 *                 de cargos e salários. Lícito quando coletivo e não lesivo.
 *  - ADMISSION  → Admissão (primeiro vínculo / registro).
 */
export enum POSITION_CHANGE_REASON {
  ADMISSION = 'ADMISSION',
  PROMOTION = 'PROMOTION',
  TRANSFER = 'TRANSFER',
  // DEMOTION é reaproveitado como "Reversão" (cargo de confiança → cargo efetivo).
  DEMOTION = 'DEMOTION',
  // ADJUSTMENT é reaproveitado como "Readaptação" (restrição médica/INSS).
  ADJUSTMENT = 'ADJUSTMENT',
  // CORRECTION é reaproveitado como "Reenquadramento" (plano de cargos).
...
// FISPQ / FDS — Safety Data Sheet (Medicina do Trabalho — chemical products).
...
// ============================================================================
// Área Andressa — Phase 1 new enums (Parts B/C/D/E)
// ============================================================================
...
// Status de um período de férias (modelo colapsado: SCHEDULED → PAID, + EXPIRED).
// "Em gozo" é DERIVADO das datas (isVacationInProgress), nunca armazenado.
export enum VACATION_STATUS {
  SCHEDULED = 'SCHEDULED', // agendada (gozo planejado; inclui "em gozo" computado)
  PAID = 'PAID', // paga / encerrada
  EXPIRED = 'EXPIRED', // período concessivo expirou sem gozo (art. 137 — dobro)
}

// "Em gozo" — estado de exibição COMPUTADO das datas (nunca armazenado):
// status === SCHEDULED e hoje ∈ [startDate, startDate + days − 1] (dias inclusivos).
// As UIs mostram um chip "Em gozo" quando true; caso contrário o rótulo do status.
export function isVacationInProgress(v: {
  status?: string | null;
  startDate?: Date | string | null;
  days?: number | null;
}): boolean {
  if (!v || v.status !== VACATION_STATUS.SCHEDULED || v.startDate == null) return false;
  ...
  const days = v.days && v.days > 0 ? v.days : 1;
  const end = startOfDay(addDays(start, days - 1));
  return today.getTime() >= start.getTime() && today.getTime() <= end.getTime();
}

// Alvo das férias coletivas (CLT art. 139): empresa toda, setores ou cargos.
...
export const DEFAULT_NOTIFICATION_SETTINGS: Record<
  string,
  { importance: string; channels: string[] }
> = {
  // Activity Alerts
  ...
  [ALERT_TYPE.STOCK_OUT]: {
    importance: NOTIFICATION_IMPORTANCE.URGENT,
    channels: [NOTIFICATION_CHANNEL.PUSH, NOTIFICATION_CHANNEL.IN_APP, NOTIFICATION_CHANNEL.EMAIL],
  },
  ...
  [ALERT_TYPE.USER_VIOLATION]: {
    importance: NOTIFICATION_IMPORTANCE.HIGH,
    channels: [NOTIFICATION_CHANNEL.IN_APP, NOTIFICATION_CHANNEL.EMAIL],
  },

  // Vacation Alerts
  [ALERT_TYPE.EXPIRING_DAYS]: {
    importance: NOTIFICATION_IMPORTANCE.NORMAL,
    channels: [NOTIFICATION_CHANNEL.IN_APP],
  },
  [ALERT_TYPE.PLANNING_CONFLICT]: {
    importance: NOTIFICATION_IMPORTANCE.HIGH,
    channels: [NOTIFICATION_CHANNEL.IN_APP, NOTIFICATION_CHANNEL.EMAIL],
  },
  ...
  [ALERT_TYPE.MISSING_DELIVERY]: {
    importance: NOTIFICATION_IMPORTANCE.HIGH,
    channels: [NOTIFICATION_CHANNEL.IN_APP],
  },

  // Supplier Alerts
  [ALERT_TYPE.DELIVERY_DELAYS]: {
    importance: NOTIFICATION_IMPORTANCE.HIGH,
    channels: [NOTIFICATION_CHANNEL.IN_APP, NOTIFICATION_CHANNEL.PUSH],
  },
};
...
// =====================
// Stock Model (per-item replenishment behavior)
// =====================
...
// Type of physical storage structure placed on the warehouse map.
...
export enum SKILL_ASSESSMENT_STATUS {
  DRAFT = 'DRAFT',
  IN_PROGRESS = 'IN_PROGRESS',
  COMPLETED = 'COMPLETED',
  CANCELLED = 'CANCELLED',
...
export enum SKILL_ASSESSMENT_PERIOD_STATUS {
  DRAFT = 'DRAFT',
  ACTIVE = 'ACTIVE',
  COMPLETED = 'COMPLETED',
  CANCELLED = 'CANCELLED',
