...
async function migrateUsers() {
  ...
  for (const user of users) {
    ...
    if (userEmail === '1603fabiorodrigues@email.com') {
      ...
      console.log(`  📌 Found 1603fabiorodrigues@email.com, will unify with 1603fabiorodrigues@gmail.com`);
    }
  }
  ...
  for (const user of users) {
    try {
      ...
      const userEmail = user.email?.toLowerCase();
      if (userEmail === 'plotter.ankaa@gmail.com') {
        console.log(`  ⏭️  Skipping plotter.ankaa@gmail.com (unified with kennedy.kobra@gmail.com)`);
      ...
      if (userEmail === '1603fabiorodrigues@email.com') {
        console.log(`  ⏭️  Skipping 1603fabiorodrigues@email.com (unified with 1603fabiorodrigues@gmail.com)`);
      ...
      const birth = employee?.birth
        ...
        : new Date(1960 + Math.floor(Math.random() * 45), Math.floor(Math.random() * 12), Math.floor(Math.random() * 28) + 1);
      ...
      if (userEmail === 'kennedy.kobra@gmail.com') {
        ...
        console.log(`  📌 Saved Kennedy's primary user ID with special config (kennedy.ankaa@gmail.com, admin sector, password set)`);
        ...
        if (plotterUserData) {
          ...
          console.log(`  🔗 Mapped plotter.ankaa@gmail.com references to kennedy.kobra@gmail.com`);
        }
      }
      ...
      if (userEmail === '1603fabiorodrigues@gmail.com') {
        ...
        if (fabioEmailComData) {
          ...
          console.log(`  🔗 Mapped 1603fabiorodrigues@email.com references to 1603fabiorodrigues@gmail.com`);
        }
      }
  ...
  try {
    ...
    if (sergioUser) {
      ...
      console.log(`  ✅ Updated Sergio (${sergioUser.name}) with admin sector, verified status, and password`);
    } else {
    ...
    if (genivaldoUser) {
      ...
      console.log(`  ✅ Updated Genivaldo (${genivaldoUser.name}) with admin sector, verified status, and password`);
    } else {
...
async function migrateItems() {
  ...
  for (const item of items) {
    try {
      if (!item.name || item.name.trim() === '') {
        console.log(`  ⏭️  Skipping item with empty name`);
        continue;
      }
      ...
      if (isPpeWithSize) {
        ...
        if (ppeItemsMap.get(baseName)!.has(normalizedSize)) {
          console.log(`  ⏭️  Skipping duplicate PPE: ${baseName} - Size ${normalizedSize}`);
          continue;
        }
      ...
      const created = await prisma.item.create({
        data: {
          ...
          isActive: analysis.isActive !== undefined ? analysis.isActive : true,
  ...
  console.log(`  📊 Items migration: ${successCount} success, ${errorCount} errors`);
  console.log(
    ...
    try {
      ...
      if (commissionField !== undefined && commissionField !== null && commissionField !== '') {
        ...
        if (commissionValue === -1) {
          ...
          console.log(`  ⚠️  Unexpected commission value: ${commissionValue}, defaulting to FULL_COMMISSION`);
          commissionStatus = 'FULL_COMMISSION';
        }
      } else {
      ...
      const task = await prisma.task.create({
        data: {
          ...
          customerId,
          sectorId: taskSectorId, // Use the user's sector
          status,
          statusOrder,
          serialNumber,
          plate,
          paintId: generalPaintingId, // This is the general painting
  ...
  for (const activity of activities) {
    try {
      ...
      const data: any = {
        ...
        itemId: idMappings.items[activity.item],
  ...
  try {
    ...
    for (const position of positions) {
      ...
      if (position.bonifiable !== shouldBeBonifiable) {
        ...
        console.log(`  ✅ Updated ${position.name}: bonifiable = ${shouldBeBonifiable} (level ${level})`);
        updatedCount++;
      }
    }
...
function getDetailedPositionLevel(positionName: string): number {
  ...
  if (normalized.includes('junior i') || normalized === 'junior' ||
      normalized.includes('júnior i') || normalized === 'júnior' ||
      normalized.includes('auxiliar i') || normalized === 'auxiliar') {
    return 1; // Junior I
  }
  ...
  if (normalized.includes('senior i') || normalized === 'senior' ||
      normalized.includes('sênior i') || normalized === 'sênior') {
    return 9; // Senior I
  }
 ...
 */
function calculateBonusValue(positionName: string, performanceLevel: number, averageTasksPerUser: number): number {
...
async function createBonusesForEligibleUsers() {
  ...
  try {
    ...
    if (eligibleUsers.length === 0) {
      console.log('  ⚠️  No eligible users found with bonifiable positions and performance level > 0');
      return;
    }
    ...
    for (let month = 1; month <= monthsToCreate; month++) {
      ...
      console.log(`    📝 Found ${allPeriodTasks.length} completed tasks for period ${startDate.toLocaleDateString()} - ${endDate.toLocaleDateString()}`);
      ...
      for (const user of eligibleUsers) {
        try {
          ...
          if (tasksForB1.length > 0) {
            ...
            console.log(`      📈 Commission breakdown: Full=${commissionCounts.FULL_COMMISSION}, Partial=${commissionCounts.PARTIAL_COMMISSION}, None=${commissionCounts.NO_COMMISSION}, Suspended=${commissionCounts.SUSPENDED_COMMISSION}, Other=${commissionCounts.other}`);
          }
          ...
          let averageTasksPerUser = eligibleUserCount > 0
          ...
          if (averageTasksPerUser === 0) {
            ...
            const demoValues = [12.5, 18.3, 25.7, 15.2, 20.8, 22.1, 16.4, 19.7, 24.3, 14.9, 21.6, 17.8];
            averageTasksPerUser = demoValues[(month - 1) % 12];
            console.log(`    💡 No actual tasks found - using demo averageTasksPerUser value: ${averageTasksPerUser} for ${month}/${currentYear}`);
          }
          ...
          console.log(`         Position: ${user.position!.name} (Level ${getDetailedPositionLevel(user.position!.name)})`);
          ...
          if (month < currentMonth ||
              (month === currentMonth && currentDate.getDate() >= 26)) {
          ...
          if (Math.random() < 0.3) { // 30% chance of having discounts
            ...
            for (let i = 0; i < numDiscounts; i++) {
              ...
              await prisma.bonusDiscount.create({
                data: {
                  ...
                  percentage: discount.percentage ? new Prisma.Decimal(discount.percentage) : undefined,
                  value: discount.value ? new Prisma.Decimal(discount.value) : undefined,
                  calculationOrder: discount.calculationOrder,
                },
              });
            }
            console.log(`      📉 Added ${numDiscounts} sample discount(s) to bonus for ${user.name}`);
          }

          console.log(`    ✅ Created bonus for ${user.name} - ${month}/${currentYear}: R$ ${bonusValue.toFixed(2)} (B1=${averageTasksPerUser})`);

        } catch (error: any) {
          console.error(`    ❌ Error creating bonus for ${user.name} - ${month}/${currentYear}:`, error.message);
        }
      }
      ...
      if (bonusesCreatedThisMonth.length > 0 && allPeriodTasks.length > 0) {
        console.log(`    🔗 Linking ${allPeriodTasks.length} tasks to ${bonusesCreatedThisMonth.length} bonuses for ${month}/${currentYear}`);
      ...
      if (bonusesCreatedThisMonth.length > 0 && eligibleUserIds.length > 0) {
        console.log(`    👥 Linking ${eligibleUserIds.length} users to ${bonusesCreatedThisMonth.length} bonuses for ${month}/${currentYear}`);
  ...
  try {
    ...
    for (let month = 1; month <= monthsToCreate; month++) {
            ...
            data: {
              ...
              discounts: {
                create: [
                  ...
                  {
                    reference: 'INSS',
                    percentage: baseRemuneration <= 1412 ? 7.5 :
                               baseRemuneration <= 2666.68 ? 9 :
                               baseRemuneration <= 4000.03 ? 12 :
                               baseRemuneration <= 7786.02 ? 14 : null,
                  ...
                  ...(baseRemuneration > 2259.20 ? [{
                    reference: 'Imposto de Renda',
                    percentage: baseRemuneration <= 2826.65 ? 7.5 :
                               baseRemuneration <= 3751.05 ? 15 :
                               baseRemuneration <= 4664.68 ? 22.5 : 27.5,
                    calculationOrder: 3
                  }] : []),
      ...
      if (monthPayrollsCreated > 0) {
        console.log(`    ✅ Created ${monthPayrollsCreated} payrolls for month ${month}/${currentYear}`);
      }
    }
