# PLANO ÚNICO (Revisão 3) — Implemento: Truck vira Implement, a arte e a série vão para o implemento, e o orçamento ganha a aprovação do valor

Data: 23/09/2026. **Revisão 2** no mesmo dia, depois das decisões do dono (DD1..DD6, bloco abaixo). **Revisão 3.1** (fim da Fase A, mesmo dia): DD11, DD12 e a confirmação da DD10 — o bloco de decisões prevalece sobre as menções a "pergunta 25/26 em aberto" que o P10/P14 ainda precisam reescrever no texto. **Revisão 3** (P00, mesmo dia): a base absorveu a `main` do deploy de 23/09, os números decisórios passaram a ser os de **produção** (`P00-producao.md`) e entraram as decisões **DD7..DD10**, com a **DD6 reescrita** (a obra da outra sessão já está em produção: caminho de contingência). Consolida os onze relatórios de mapeamento desta pasta `docs/implemento/` — `01-dados-e-migracao.md` … `07-contratos-invisiveis.md` (v1) e `08-estado-orcamento-api.md`, `09-estado-orcamento-clientes.md`, `10-serie-api.md`, `11-serie-clientes.md` (Revisão 2) — e a conferência que fiz no código onde eles divergiam. Até a Revisão 2 era **somente plano**. No P00 (Revisão 3) houve só o merge da `main` na base (api e web), a branch nova do app e a medição de produção **somente leitura**; nenhum código do rework foi escrito.
...
| **DD7** (Revisão 3) | **Cobrança só depois de assinado.** Aprovar a cobrança (fatura, NFS-e, boleto) exige `Budget.status = APPROVED` **e** `signatureStatus = SIGNED`. O legado migrado como `WAIVED` continua faturável. O ato manual "assinado fora do sistema" é **pergunta em aberto** (pergunta 25): **não implementar** | Cai a D-30 ("a cobrança abre no valor aprovado") e a pergunta 3. O portão da cobrança ganha a condição `signatureStatus ∈ {SIGNED, WAIVED}` no ponto único que já existe (`budget.service.ts` `internalApprove`, a checagem `status === APPROVED`, `:4229` depois do merge do P00). Sai o ato manual "Dispensar assinatura" (`PUT /budgets/:id/signature-waiver`): `WAIVED` só nasce na migração e na conciliação (D-34). Produção: dos 536 aprovados, 504 sem coleta viram `WAIVED` (faturáveis) e 30 com COMPLETED viram `SIGNED`; **o nº 885** (coleta vencida, cobrança pendente) fica travado até reassinar — triagem `LEGACY_APPROVED_UNSIGNED` | D-28, D-30, D-34; §2A.2, §2A.4, §2A.8; §4.3 (M3o); §5.1; §7.6; §8 (G34); §12 |
...
| **DD10** (Revisão 3) | **A liberação manual "Disponibilizar para produção" respeita a arte** (recomendação do plano; **confirmar com o dono antes do P12**) | A pergunta 23 vira recomendação adotada, com confirmação marcada no P12. Sem dispensa (DD9), não há "saída registrada" para furar o portão: quem precisa liberar sem a arte do cliente aprova "em nome do cliente" (nota obrigatória) | D-15; §6.2; §9 (P12); §12 |
| **DD11** (Revisão 3.1, respostas do dono em 23/09 à tarde) | **Existe o ato "Assinado fora do sistema"** (resposta à pergunta 25): COMMERCIAL/ADMIN registram que o cliente assinou por fora (papel, WhatsApp), com **nota e anexo obrigatórios** (foto ou PDF da assinatura); o eixo da assinatura passa a um estado que **libera a cobrança** como `SIGNED`, mas distinguível dele na tela ("Assinada fora do sistema") e na trilha. O nº 885 sai da triagem por esse caminho | Onde este plano diz "o ato manual 'assinado fora do sistema' é a pergunta 25 / não implementar" (DD7, D-28, D-30, D-34, §2A, §12), **vale esta linha**; o P14 desenha o valor do enum (novo valor × `SIGNED` com `source = OFFLINE`) e o P10 o inclui na migração do tipo | D-28, D-30, D-34; §2A.4, §2A.8; P10, P14 |
| **DD12** (Revisão 3.1) | **Pedido de compra na assinatura: vale a regra da `main`** (resposta à pergunta 26): **quem TEM o papel PURCHASING** (mesmo acumulando outros) só assina informando o nº do pedido de cada veículo, **nas duas cerimônias** (página pública OTP e sessão do portal). O `purchase-order-gate` da branch (só quem é SÓ Compras) passa a seguir a regra da `main` | Um só predicado para as duas cerimônias, com teste | portal (P13b) e assinatura (P14) |
| **DD10 confirmada** (Revisão 3.1) | O dono **confirmou**: a liberação manual "Disponibilizar para produção" exige a arte aprovada (só trabalho novo) | Tirar o "confirmar com o dono antes do P12" de D-15, §6.2, §9 e §12 | P12 |
...
| **Estado da assinatura** (eixo próprio) | `Budget.signatureStatus` (`BudgetSignatureStatus`): `NOT_ISSUED`, `AWAITING_CUSTOMER`, `AWAITING_ANKAA`, `SIGNED`, `REFUSED`, `EXPIRED`, `INVALIDATED`, `WAIVED` | **Assinatura**: Não emitida, Aguardando assinaturas, Falta a Ankaa, Assinada, Recusada, Vencida, Invalidada, Dispensada (legado) | ORÇAMENTO (escrito só pela coleta; `WAIVED` só pela migração e pela conciliação) | Não é `SignatureEnvelope.status` (o envelope é a coleta; o eixo é o resumo do orçamento). `SIGNED` (ou `WAIVED` do legado) é o que libera a cobrança (DD7) |
...
| **Assinatura dispensada (legado)** | `signatureStatus = WAIVED`, escrito **só** pela migração (aprovados sem coleta nenhuma: 504 em produção) e pela conciliação que cria orçamento já aprovado e pago (D-34) | **Dispensada (legado)** | sistema | Não é ato de tela: "Dispensar assinatura" **não existe** nesta entrega (DD7); um ato manual "Assinado fora do sistema" é a pergunta 25. **"Dispensar arte" também não existe** (DD9) |
...
| V12 | Quem libera a cobrança: `08` P3 "valor aprovado **não** fatura" × `09` P1 "valor aprovado fatura" | **DECIDIDO (DD7, Revisão 3): só depois de assinado.** O portão da cobrança (`internalApprove`, `status === APPROVED`, `budget.service.ts:4229` depois do merge do P00) ganha a condição `signatureStatus ∈ {SIGNED, WAIVED}` no mesmo ponto, sem remodelar; `WAIVED` é só o legado migrado e a conciliação. Em produção, 504 dos 536 aprovados não têm coleta (viram `WAIVED`, faturáveis) | `budget.service.ts:4229`; `P00-producao.md` §4 |
...
| D-30 | **Onde o Billing nasce e quando se cobra** | O `Billing` continua nascendo na **criação** do orçamento como plano (`budget.service.ts:622-660`; `budget-customer-config-sync.ts:890,996`; cópia `task.service.ts:13458`; conciliação `receivable-task-match.service.ts:988`) — nada muda aí. **A aprovação da cobrança muda (DD7, Revisão 3):** exige `status === APPROVED` **e** `signatureStatus ∈ {SIGNED, WAIVED}` no ponto único que já existe (`internalApprove`, `budget.service.ts:4229` depois do merge do P00; rotas `PUT /billings/:id/approve` e a antiga por tarefa chegam nele). Recusa: 400 "A cobrança só pode ser aprovada depois da assinatura do orçamento." O faturamento mostra o selo "Assinatura: …" e o filtro. Corrigir junto o filtro **negativo** do financeiro (`schemas/task.ts:1775`, `notIn [PENDING, SIGNED, EXPIRED]`) para **positivo** `APPROVED` (X6). As telas de faturamento (web, app) desabilitam "Aprovar" com o motivo quando o eixo não é `SIGNED`/`WAIVED` | Decisão do dono. Mudar quando nasce o Billing reabriria o P2 de 17/09 sem pedido. Impacto medido em produção: 504 aprovados sem coleta → `WAIVED` (faturáveis); 30 com COMPLETED → `SIGNED`; **nº 885** (coleta vencida, cobrança pendente) trava até reassinar (triagem `LEGACY_APPROVED_UNSIGNED`) | **DECIDIDO (DD7)**; "assinado fora do sistema" [pergunta 25] |
...
| ~25 pontos que tratam `APPROVED` como "pode faturar" (`09` §4) | nada muda | nada muda | os leitores não mudam; o **portão** da cobrança ganha `signatureStatus ∈ {SIGNED, WAIVED}` (DD7), num ponto só |
...
EIXO DO VALOR — Budget.status                                   M = manual (interno)  P = portal  S = sistema
 ...
 interno ▶ PENDING ──┴───────────────────────────────────────────▶ IN_NEGOTIATION ──"Aprovar valor" (P, APPROVE_VALUE)──▶ APPROVED ──▶ cobrança SÓ com
             ▲   │                                                     │                                                    │   signatureStatus SIGNED/WAIVED (DD7)
...
EIXO DA ASSINATURA — Budget.signatureStatus (anda com Budget.status = APPROVED; as coletas legadas andam sobre PENDING)
 ...
 SIGNED (ou WAIVED do legado) ─▶ a cobrança pode ser aprovada (DD7)
...
| Valor | Rótulo | Escritor (sempre na transação do envelope, salvo `WAIVED`) | Efeitos |
|---|---|---|---|
...
| `WAIVED` | Dispensada (legado) | **só** a migração (aprovados sem coleta nenhuma; 504 em produção) e a conciliação que cria orçamento já pago (D-34). **Nenhum ato de tela** escreve `WAIVED` (DD7; "assinado fora do sistema" é a pergunta 25) | libera a cobrança como `SIGNED`; a emissão continua possível (volta a `AWAITING_CUSTOMER`) |
...
D-30. Resumo: **nasce na criação** do orçamento, como plano (FATO: os 15 `REQUESTED` do clone já têm `Billing`); a **aprovação** da cobrança (fatura, NFS-e, boleto) exige `Budget.status = APPROVED` **e** `signatureStatus ∈ {SIGNED, WAIVED}` — **cobrança só depois de assinado** (DD7), com o legado migrado `WAIVED` faturável. Um ponto só no código (`internalApprove`, `budget.service.ts:4229` depois do merge do P00, onde já está a checagem de `APPROVED`); as rotas `PUT /billings/:id/approve` e a antiga por tarefa chegam nele. O faturamento **mostra** o selo "Assinatura: …" no cabeçalho do assistente (`web pages/financial/billing/details/[id].tsx`) e na lista, e desabilita "Aprovar" com o motivo. Não existe ato manual que libere a cobrança sem a coleta ("assinado fora do sistema" é a pergunta 25). O filtro negativo do financeiro vira positivo (X6). A cascata `Billing ↔ Budget` (`billing-status-cascade.service.ts:127`) não muda. Liquidar e reverter uma cobrança **já aprovada** não passam pelo portão novo (a trava é na aprovação).
...
| APPROVED, EXPIRED | **2** (nº 741 com cobrança PARTIAL; **nº 885** com cobrança PENDING) | 1 | APPROVED | `EXPIRED` | `MIGRATED` | **travada até reassinar** — triagem `LEGACY_APPROVED_UNSIGNED` (o nº 741 já faturou; o 885 precisa de coleta nova ou da resposta à pergunta 25) |
...
+  /// para Implement.projectFiles (migração 20260924120200).
+  projectFiles          File[]                @relation("TASK_PROJECT_FILES")
 }
...
+  /// do envelope; WAIVED só pela migração e pela conciliação (DD7). SIGNED/WAIVED liberam a cobrança. Guarda G29.
...
+enum BudgetSignatureStatus { NOT_ISSUED  AWAITING_CUSTOMER  AWAITING_ANKAA  SIGNED  REFUSED  EXPIRED  INVALIDATED  WAIVED }
...
| `BudgetSignatureStatus` (novo) | NOT_ISSUED, AWAITING_CUSTOMER, AWAITING_ANKAA, SIGNED, REFUSED, EXPIRED, INVALIDATED, WAIVED | Não emitida, Aguardando assinaturas, Falta a Ankaa, Assinada, Recusada, Vencida, Invalidada, Dispensada (legado) | `CREATE TYPE` em M3o (tipo criado e usado na mesma transação: permitido, só `ADD VALUE` tem a restrição) |
...
### 4.1 Releases e migrações (datas **depois** de `20260923120000`, a migração da outra branch)

| Migração | Release | Conteúdo | Destrutiva? | Pode ir antes do código? |
|---|---|---|---|---|
| **M0** `20260924100000_arte_estados_e_tipos` | **R-A** (antecipada) | `ALTER TYPE "LayoutStatus" ADD VALUE 'PENDING_APPROVAL'`, `ADD VALUE 'SUPERSEDED'`; `CREATE TYPE "LayoutApprovalSource"` (com os 5 valores, inclusive `MIGRATED_ENVELOPE`); `CREATE TYPE "RearDoorLeaves"` | não | **sim**: o código velho ignora valor e tipo novos. Vai antes, para que o ensaio de M1–M3 rode numa transação revertida (valor novo de enum não pode ser usado na mesma transação em que nasce) |
| **M1** `20260924120000_truck_vira_implement` | R-B | RENAME de tabela, constraints, índices, coluna `implementType`→`type`, tipo `TruckCategory`→`ImplementCategory` | quebra o processo velho | não. Build antes; API **parada** durante a migração |
| **M1s** `20260924120050_serie_no_implemento_e_implemento_obrigatorio` | R-B | Implemento para **toda** tarefa sem (produção 23/09: 1.676; clone: 1.678; `spot NULL`, datas da tarefa); `Implement.serialNumber` + cópia + `@unique` + coluna gerada + GIN; `DROP INDEX "Task_serialNumber_key"`; gatilhos do espelho e de somente leitura; `CONSTRAINT TRIGGER` diferidos de "ao menos um implemento"; `spot` sem default | não (aditiva; o espelho mantém `Task.serialNumber`) | **não**: o código velho grava `Task.serialNumber` (o gatilho recusaria) e cria tarefa sem `truck` (o gatilho diferido recusaria no COMMIT, `task-prisma.repository.ts:1061-1074`). Vai com o código novo, API parada |
| **M2** `20260924120100_implemento_frente_e_porta_traseira` | R-B | `frontSideMeasureId` + FK + índice; 3 colunas da porta + CHECKs; desfaz as medidas compartilhadas | não | depende de M1 (nome da tabela) |
| **M3** `20260924120200_arte_do_implemento_e_projeto_da_tarefa` | R-B | Arquivo morto; colunas novas de `Layout`; `_IMPLEMENT_PROJECT_FILES`; `LayoutDecision`; projeto do implemento; PDFs → projeto da tarefa; fan-out das imagens; **arte do orçamento com preservação de `File.id` (M-A..M-D)**; O.S. em `WAITING_APPROVE`; órfãos; constraints; **DROP `_TaskLayouts`**. O passo 1b vira no-op (M1s já criou todos os implementos), mas fica, idempotente | sim (M2M) | não |
| **M3o** `20260924120300_orcamento_valor_aprovado_e_eixo_da_assinatura` | R-B | `BudgetValueApproval` + tipos novos; `Budget.signatureStatus`; `BudgetStatus` recriado sem `PRE_APPROVED` (com `queueRank` derrubado e recriado); backfill de estado, eixo e aprovação (§2A.10); `statusOrder` renumerado e default 3 | sim (tipo recriado) | não: o código velho escreve `PENDING` na emissão e lê `PRE_APPROVED` |
...
```sql
-- M0 — 20260924100000_arte_estados_e_tipos  (release R-A, antecipada; aditiva)
...
```sql
-- M1 — 20260924120000_truck_vira_implement  (catálogo apenas; nenhum byte de dado muda)
...
```sql
-- M1s — 20260924120050_serie_no_implemento_e_implemento_obrigatorio  (R-B, logo depois de M1)
...
```sql
-- M3o — 20260924120300_orcamento_valor_aprovado_e_eixo_da_assinatura  (R-B, depois de M3; DD2, §2A)
...
CREATE TYPE "BudgetSignatureStatus" AS ENUM
  ('NOT_ISSUED','AWAITING_CUSTOMER','AWAITING_ANKAA','SIGNED','REFUSED','EXPIRED','INVALIDATED','WAIVED');
...
--    Vai à triagem para o dono decidir caso a caso (pergunta 25).
...
| Aprovar a cobrança de orçamento não assinado (DD7) | aprova | **400** "A cobrança só pode ser aprovada depois da assinatura do orçamento." (`WAIVED` do legado passa) |
...
| `budget.service.ts:3872-3895` (`internalApprove`: só fatura `APPROVED`; `:4229` depois do merge do P00) | | **muda (DD7, D-30)**: exige também `signatureStatus ∈ {SIGNED, WAIVED}`; 400 "A cobrança só pode ser aprovada depois da assinatura do orçamento."; guarda G34 |
...
| `pages/financial/billing/details/[id].tsx:1527-1538` (pin no faturamento) | | **fica**; + selo "Assinatura: …" no cabeçalho e "Aprovar" desabilitado com o motivo quando a assinatura não é `SIGNED`/`WAIVED` (DD7) |
...
| 7 | **Aprovar a cobrança** (fatura, NFS-e, boleto) | interno: FINANCIAL, ADMIN | faturamento (`PUT /billings/:id/approve`) | `Billing` PENDING → APPROVED | **só com `signatureStatus = SIGNED`** (ou `WAIVED` do legado) (DD7); antes disso o botão fica desabilitado com o motivo e a API recusa |
...
| `APPROVED` | `SIGNED` ou `WAIVED` (legado) | — | Em andamento | Daqui em diante quem anda é a produção e a cobrança (a cobrança só abre aqui, DD7). |
...
| **G34** | **Cobrança só depois de assinado** (DD7, Revisão 3) | aprovar fatura/NFS-e/boleto de orçamento com valor aprovado e assinatura não concluída | `internalApprove` pelos três caminhos (`PUT /billings/:id/approve`, `PUT /budgets/:id/internal-approve`, `…/internal-approve/:taskId`): `signatureStatus` `NOT_ISSUED`, `AWAITING_*`, `REFUSED`, `EXPIRED`, `INVALIDATED` → 400 com a frase da DD7; `SIGNED` e `WAIVED` → passa; liquidar/reverter cobrança já aprovada não passa pelo portão; o app 1.4.1 recebe a mensagem em toast | P14 |
...
| **P06** — release R-A (**sozinho**, curto) | M0 (com `MIGRATED_ENVELOPE`) + enums no schema; G3 (censo) e log de `X-App-Version`; deploy R-A (build antes, `systemctl`) | `prisma/migrations/20260924100000…`, `schema.prisma` (enums), middleware | M0 aplicada em produção; censo gravando | boot + e2e-portal |
| **P10** — schema e migração (**sozinho**, api) | §3 inteiro no `schema.prisma` (implemento com série, espelho, `spot` sem default, arte, `BudgetValueApproval`, `signatureStatus`, `BudgetStatus` sem `PRE_APPROVED`) e os espelhos TS do enum (`src/constants/enums.ts:2802-2857`, `sortOrders.ts:186-201`); **M1, M1s, M2, M3 (passo 5 com M-A..M-D), M3o** (§4); script de ensaio (`scripts/rehearse-implement-migration.ts`: `$transaction` → M1…M3o → invariantes → régua G11 → `throw`); triagem `LEGACY_APPROVED_UNSIGNED` (DD7); sem colunas de dispensa de arte (DD9) e sem hash na `BudgetValueApproval` (DD8) | `prisma/schema.prisma`, `prisma/migrations/2026092412…`, `src/constants/{enums,sortOrders}.ts`, scripts de ensaio | `prisma validate` ok; `migrate status` sem deriva; **ensaio verde no clone**; triagem gerada | ensaio + invariantes (§4.6) + G20 (lado banco) + G25 |
| **P11** — API: Implement, série e janela bilíngue (**sozinho**, api; pode ser P11a "rename + compat + série" → P11b "frente + porta") | §6.1 inteiro; tradutor `translateLegacyImplementKeys`, `ImplementLegacyMirrorInterceptor`, sintetizador de `task.layouts` (no P12); rotas `/implements` + alias `/trucks`; `TASK_FIELD_DOMAINS.implement` junto com o zod (`implement.serialNumber` exige `identity`); whitelist; `paint.service.ts` e `file-reference.service.ts` no mesmo commit; frente e porta no writer (4 faces), multipart `frontSide` nos 3 interceptors, `side` validado, cotador com FRENTE; chaves do snapshot congeladas no `build()` (inclusive a série lida do implemento); **série**: W1–W3, W6, as 4 unicidades, `truck/implement: null` → 400, criação **sempre** com implemento, ramos "cria se não existir" apagados, `implementUpdateSchema` sem série, tracker e changelog `TASK/serialNumber`, `dps.builder.ts:522-524` (comentário) | §6.1, §6.13 (API, R-B), `quote-snapshot.service.ts:92-120,176-182,342,436,488-499,598-626,832-840`, `quote-text.ts`, `dossier-assembler.service.ts` | tsc sem erro (G0); G6a caindo; G24 ligada; **nenhum 403/500** com as fixtures dos clientes velhos (G4) | G2, G7, G12, G15 com 4 faces, G16, **G19, G20, G22, G23, G32**, legado (`truck{…implementType}` grava em `implement.type`; include `truck` devolve `truck` e `implement`; os dois juntos → 400), rollback de `TRUCK/implementType` e de `TASK/serialNumber`, notificação `task.field.truck.*` e `task.field.serialNumber` continuam disparando, busca de tinta por placa e por série |
| **P12** — API: arte do implemento, R1 e arte na assinatura (∥ P13a; api) | `ImplementLayoutService` + rotas `/implements/:id/layouts/*` (**sem** dispensa de arte, DD9); eventos com ator discriminado; lembrete; portão de liberação (D-15, DD3), inclusive a liberação **manual** (DD10 — **confirmar com o dono antes de começar**), e o fechamento da O.S. "Aprovar com o Cliente" e da O.S. de ARTE em `WAITING_APPROVE`; escritores do layout por veículo da `main` saem (§6.2 item 26b), `layoutGateFailure` passa a falar da arte do implemento; todo o R1 (§6.2 itens 1–32, exceto M4, `budget.guards.ts:215` e os itens 17/18/32, que são do P14); **`quoteArtworkOf`** alimentando snapshot (v7, `layoutFileIds` + `layoutCoverage` condicional), documento (legenda por veículo) e diff; toda mudança de estado de arte chama `onQuoteContentChanged`; `file-reference` sem saída; contextos de arquivo; sintetizador legado de `task.layouts`; tratamento no-op/400 de `layoutIds` | §6.2, `portal-read.service.ts:1048,1115-1117`, `portal-projection.service.ts:233,576,790-791` | **G11 verde: quem casava antes casa depois**; `Budget.layoutFiles` fora do Prisma; exclusão de arquivo funciona | G11, `quote-diff` (coverage condicional), `quote-merge-rules`, `signature-refusal`, arte: send/approve-on-behalf (nota)/reprove/new-version/bulk, liberação automática e manual só com arte (trabalho novo), G11 com `PER_VEHICLE`, G33 (lado arte) |
| **P13a** — API portal: frente, porta, projeto, série (∥ P12; api) | §6.3 nas linhas de medida, porta, projeto, `implementVinPlate`, `TASK_BASE_SELECT`, projeção `implement`, `mexeNoCaminhao`; **W4/W5** (série do portal no implemento, `spot: null`); trava de edição em `IN_PRODUCTION` (DD5); docs do contrato | `portal-identity.*`, `portal-request.*`, `schemas/portal-request.ts`, `portal-read.service.ts:202-229`, `portal-projection.service.ts:197-199,464-466,841-867`, `portal-vehicle-identity.ts` | "PATCH só com `medidas.frente` grava"; porta fora da faixa → 400; série do portal cai no implemento com `field: 'serialNumber'` nos conflitos | `portal-identificacao`, `portal-requisicao`, e2e-portal 01 (+ frente, + série no implemento) |
| **P14** — API: máquina do orçamento (∥ P13b; api) | §2A inteiro e §6.14 (API): `BudgetValueApproval` como registro (sem hash, DD8; fecha a vigente quando o orçamento sai de `APPROVED`); atos (send-to-customer, value-approval, reprovar, retirar — **sem** "dispensar assinatura", DD7); **portão da cobrança** `signatureStatus ∈ {SIGNED, WAIVED}` em `internalApprove` (DD7, G34); grafo em duas tabelas; `budgetApprove` como "aprovar valor"; eixo `signatureStatus` escrito na transação do envelope (emissão, grupo 0, conclusão, recusa, vencimento, invalidação); `assertEmissionReady` nos três lugares + `gates` + `emission`; conclusão sem portão; `tolerateArtworkAfterSeal`; `LEGACY_APP`; nascimento decidido no servidor; X1–X9; portal: `portal-decision.*` (aprovar valor, recusa → PENDING, sem fabricar requisição); notificações novas no seed | `budget.service.ts`, `budget.guards.ts`, `budget.controller.ts`, `budget.module.ts`, `signature-envelope.service.ts`, `signature-expiry.scheduler.ts`, `portal-decision.*`, `attention.service.ts`, `dashboard-prisma.repository.ts`, `invoice-analytics.service.ts`, `schemas/{budget,task}.ts`, `service-order.service.ts:209-230,1897`, `task.service.ts:808-820,11060-11086` | máquina do §2A.5 inteira por teste; nenhum escritor de `Budget.status` fora da tabela; o app 1.4.1 aprova e reprova pela janela | **G26–G29, G31**, G33 (lado coleta), **G34**, `test:portal-cliente:boot`, e2e-portal 03/04 reescritos (valor → arte → emitir → assinar) |
| **P13b** — API portal: aprovar a arte e o valor na tela (∥ P14; api) | `APPROVE_ARTWORK`; rename `PRE_APPROVE → APPROVE_VALUE` (D-35) em `portal-capabilities.ts`; `portal-artwork.controller/service`; resumo (`valueApproval`, `artworks`, `signatureStatus`); projeção do orçamento com a arte dos implementos; notificação ao contato; seed da demo | §6.3 nas linhas de arte e de resumo, `portal-notification.service.ts`, `scripts/seed-portal-demo.ts` | tabela-verdade das capacidades; 404 fora do escopo; motivo obrigatório; lote atômico; 409 em decisão repetida | novo `test:portal-arte`; `portal-recorte` com as colunas novas; `test:portal-cliente:boot` com as rotas novas; G17 (lado API) |
...
| Orçamento | `api/tests/budget-state-machine.test.ts`, `budget-value-approval.test.ts`, `emission-gate.test.ts` | §2A.5 inteira (manuais × sistema); editar o valor como hoje — auto-revert sem status fixado, status fixado mantém `APPROVED`, a aprovação vigente fecha ao sair de `APPROVED` (DD8; a G30 foi retirada); cobrança só com `SIGNED`/`WAIVED` (G34); portão igual nos três caminhos (G28); `blockers` string (G27); eixo coerente (G29); arte que muda com coleta RUNNING/COMPLETED (G33); X1–X9 | P14 |
...
| 45 | (auditoria §14) **Portão da DD3 que não existe**: a regra de liberação é função pura sem acesso à arte; com a linha "sem mudança de código", o portão nunca seria escrito, ou seria posto em cada O.S. (o que a DD3 descartou) | alta | média | §6.2 reescrito (argumento `artworkGate` + 6 chamadores); a liberação manual respeita a arte (DD10, a confirmar antes do P12) |
...
| 25 | (Revisão 3, DD7) **"Assinado fora do sistema"**: com a cobrança só depois da assinatura, haverá um ato manual que libere a cobrança quando o cliente assinou em papel, por e-mail ou por outro sistema? Se sim: quem pode (COMMERCIAL e ADMIN?), com nota obrigatória, e **exige anexar o documento assinado**? Vale também para o nº 885 (coleta vencida, cobrança pendente) e para o nº 741? E a conciliação, que cria orçamento já aprovado quando o dinheiro já entrou, continua gravando "Dispensada (legado)" sozinha? | **Sim, com anexo obrigatório** (COMMERCIAL, ADMIN; grava `WAIVED` com a nota e o arquivo no `ChangeLog`); a conciliação continua como está. **Não implementar até a resposta** (DD7) | P14 |
| 26 | (Revisão 3, merge do P00) **Pedido de compra na assinatura**: hoje convivem duas regras. Na página pública (código por WhatsApp/e-mail), quem **tem** o papel Compras informa o nº do pedido de cada veículo na hora de assinar (regra da `main`, em produção). No portal (sessão), quem tem **só** o papel Compras não assina sem o pedido no veículo (regra da branch). Qual vale para os dois canais: "tem Compras" ou "só Compras"? E o signatário do portal também pode informar o número na própria cerimônia? | **Uma regra só**: "tem Compras" e informar na cerimônia, nos dois canais (é a que está em produção e não trava quem acumula papéis sem dar saída) | P13b/P14 |
...
|---|---|---|
| 23 | A liberação **manual** para produção também exige a arte aprovada? | **Respondida na Revisão 3 (DD10)**: sim, respeita a arte (confirmar com o dono antes do P12); não há "Dispensar arte" (DD9) |
...
- **Condições:** somam-se às seis das §13.5/§13.6 mais duas:
  - **(7)** as perguntas 23 e 24 respondidas antes do P12 (Revisão 3: a 24 pela DD3; a 23 pela DD10, com confirmação marcada no P12);
