# Rework Implemento — ESTADO (25/09/2026, noite — par [P14 ∥ P13b] fechado e integrado, na nuvem)
...
- `ankaa_implemento`: banco da Fase B. Tem M0 + M1 + M1s + a nomenclatura (`20260930120060`, Mnom) + a série só no implemento (`20260930120070`, M5s) + **M2** (`20260930120100`) + os avisos da porta (`20260930120110`) + **M3** (`20260930120200`, aplicada em 25/09 ~09h — é o CORTE do portão da arte neste banco) + `LayoutApprovalSource.INTERNAL` (`20260930120210`) + as 6 migrations da main de 24/09.
...
- **P12.2 + P12.3 — a arte do implemento e o portão da liberação** (`4582d732`, contrato `bf7f4848`), régua 32/32 (197 s):
  - **Portão da arte (P12.3, DD3/DD9/DD10)** — `src/utils/artwork-gate.ts`: `artworkGate` = `OK` | `PENDING` | `NOT_APPLICABLE`. "Trabalho novo" = a primeira O.S. de ARTE da tarefa (ou a própria tarefa, se não tem O.S. de arte — DD9) nasceu depois da **aplicação da M3 neste banco** (`_prisma_migrations.finished_at`, casado pelo sufixo `_arte_do_implemento_e_projeto_da_tarefa`, que sobrevive ao re-carimbo do P30); `ARTWORK_GATE_SINCE` (ISO) sobrepõe, para banco montado com `db push`; sem corte, não se aplica. `calculateCorrectTaskStatus` e `getTaskUpdateForLayoutServiceOrderStatusChange` recebem o portão (obrigatório: o `tsc` apontou os 5 chamadores); as 4 liberações escritas à mão (2 no `service-order.service`, 2 no `task.service`) também; `isReadyForProductionRelease` é a regra num lugar só. A **liberação manual** (update, lote, desfazer do histórico — `/start` passa pelo update) recusa com 400 quando o status ENTRA em produção e o portão é `PENDING`. `getTaskUpdateForServiceOrderStatusChange` NÃO recebeu o argumento (o plano pedia): ela só trata O.S. de PRODUÇÃO e nunca tira a tarefa de PREPARATION, e arte aprovada não se desaprova (D-21). Tabela pura: `tests/artwork-gate.test.ts` (32).
  - **`ImplementLayoutService` (P12.2)** + rotas: `GET/POST /implements/:id/layouts`, `POST …/:layoutId/send`, `…/approve-on-behalf` `{note}`, `…/reprove` `{note}`, `…/new-version` (multipart `files`), `DELETE …/:layoutId` (só rascunho), `POST /implements/layouts/bulk` `{implementIds, fileId}`. Papéis: editar/enviar = DESIGNER, COMMERCIAL, ADMIN; decidir = COMMERCIAL, ADMIN; ler = os do implemento + DESIGNER, com o filtro por papel também no include `layouts` do `GET /implements` (risco 25). `approveFromPortal`/`reproveFromPortal` prontos para o P13b (ator `RESPONSIBLE`; nunca id de contato em campo de `User`).
  - Cada decisão: `updateMany … where status` (corrida → 409, G17), `LayoutDecision`, `fileSha256` dos bytes no ato, versão nova aprovada deixa a anterior `SUPERSEDED` (D-21). **Enum novo `LayoutApprovalSource.INTERNAL`** (migration `20260930120210_decisao_interna_da_arte`): a trilha precisava de uma origem para enviar, reprovar internamente e substituir — marcar ON_BEHALF mentiria ("em nome do cliente"). `Layout.approvalSource` continua só com quem aprovou.
  - Depois da decisão (cada efeito na sua transação; nenhum desfaz a decisão): enviar → "Aprovar com o Cliente" aberta vai a `WAITING_APPROVE` e o contato é avisado no portal (`layout.portal_pending_approval`, contatos ATIVOS da tarefa com papel que aprova arte — a lista do §7.2; o P13b troca pela capacidade `APPROVE_ARTWORK`); aprovar → "Aprovar com o Cliente" e as O.S. de ARTE em `WAITING_APPROVE` vão a `COMPLETED` **pelo `ServiceOrderService`** (tempo, trilha e liberação automática reaproveitados), a tarefa é liberada se a arte era a última peça (`task.status.changed`), e o orçamento passa pelo `onQuoteContentChanged`; reprovar → essas O.S. voltam a `IN_PROGRESS`. Eventos `artwork.approved`/`artwork.reproved` com ator discriminado.
  - **D-31 (`tolerateArtworkAfterSeal`)**: contrato `COMPLETED` cuja ÚNICA mudança material é a arte (`layout`/`layoutCoverage`) não cai — grava deriva "Arte alterada depois da assinatura". Coleta `RUNNING` cai como antes. O aditivo de arte opcional fica para depois (P14/P26).
  - Lembrete diário também avisa o contato (`layout.portal_pending_approval_reminder`). `LAYOUT_STATUS` com 5 estados e `LAYOUT_APPROVAL_SOURCE` com rótulos no contrato.
  - `tests/implement-layout.test.ts` (36): máquina do §7.1, corrida, "aprovada não se reprova", versão nova, portal, lote atômico, portão (O.S. de arte concluída não libera; manual → 400; aprovar libera sozinho; legado livre), G33 lado arte (COMPLETED fica com deriva; RUNNING cai).
  - ⚠️ **Impacto em quem cria tarefa nova**: todo teste/fluxo que conclui O.S. de ARTE esperando WAITING_PRODUCTION agora precisa da arte aprovada (trabalho novo). O e2e do portal e o web (P21) têm de aprovar a arte antes.
  - Fica para depois: `task_quote.ready_for_signature` quando a última arte chega (é da máquina do orçamento, P14); o aditivo de arte (D-31); textos "arte do implemento" em `notification-template.service.ts` (chaves `task.layout.*` ficaram sem emissor).

- **P14 — a máquina do orçamento no Modelo C** (`8c2e327`; detalhe em `notas/P14.md` §Resultado):
  - Dois eixos: o **valor** (`Budget.status`, grafo em duas tabelas manuais × sistema em `budget-transitions.ts`, exportadas no contrato — G26) e a **assinatura** (`Budget.signatureStatus`, escritor único `writeSignatureAxis` na transação do envelope: emissão, grupo 0, conclusão, recusa, vencimento, invalidação, cancelamento). A emissão não escreve mais `Budget.status` (X1); `replayCompletion` repara o X9.
  - Atos do valor: `PUT /budgets/:id/send-to-customer`, `…/withdraw-from-customer`, `PUT/DELETE …/value-approval` (nota/motivo obrigatórios), `/status` delegando aos atos (X7), `budget-approve` sem nota = `LEGACY_APP`. A gravação genérica não chega mais a `APPROVED` (400). Nascimento decidido no servidor (D-34). `BudgetValueApproval` acompanha o status por um reconciliador só (`utils/budget-value-approval.ts`), chamado por todos os escritores de status; a reativação pela O.S. não ressuscita `APPROVED`.
  - Portão de emissão E1–E6 (`utils/emission-gate.ts`) nos três lugares, `gates` ao lado de `blockers: string[]` (G27/G28); `emission` e `valueApproval` no detalhe; "pronto para emitir" (`task_quote.ready_for_signature`) pelos dois lados (valor chega / última arte chega).
  - **Cobrança só depois de assinado** (DD7): predicado único `isBillableSignatureStatus` em `internalApprove`, `billable` na leitura, filtro positivo do financeiro (X6), `?signatureStatuses=` no faturamento (G34).
  - **DD11** "Assinado fora do sistema": `POST /budgets/:id/offline-signature` (nota + anexo, E1 ∧ E3, eixo `SIGNED_OFFLINE`, trilha com a nota; sair de APPROVED invalida e fecha).
  - **DD12**: `orderNumberRequirement` nas duas cerimônias (quem TEM Compras; número ∨ `purchaseOrderId`; 400 com a frase da `main`; o portal recebe `orderNumbers[]`); `purchase-order-gate.ts` apagado (G35).
  - Portal, escrita: `aprovar-valor` → `APPROVED` com `BudgetValueApproval{PORTAL}`; `recusar` → `PENDING`; sem fabricar `BudgetRequest`.
  - **M3o-b promovida** (`20260930120350`): `BudgetStatus` sem `PRE_APPROVED`, `statusOrder @default(3)`, `queueRank` novo; zod derivado do enum (G31); seed com `task_quote.value_approved`/`ready_for_signature`; Atenção `budget.ready-to-emit` (gêmeo do web no P22).
  - Testes novos: `budget-state-machine` (45), `emission-gate` (48), `offline-signature` (23); `portal-decisao`, `portal-assinatura-compras` (G35) e o e2e `03` reescritos.
- **P13b** (`83951ef`, agente no worktree `impl/p13b`, banco `ankaa_implemento_p13b`; detalhe em `notas/P13b.md`): capacidade `APPROVE_ARTWORK`, rotas de arte do portal (lista, aprovar, reprovar com motivo, lote), leitura das artes/`valueApproval`/`signatureStatus`/`artwork`/resumo, marco "Arte aprovada", aviso ao contato pela capacidade, demo e contrato. `portal-arte` 71/71, `portal-recorte` 178/178.
- **Integração do par** (merge `81e34c4` + commit de integração): disjunção vazia; lote de arte do portal ATÔMICO de verdade (`approveManyFromPortal` numa transação); `emission` no detalhe do orçamento do portal em língua de cliente (`portal-emission.ts`, `test:portal-emissao`); contrato do portal com o trecho do P14; G36 `--final` = 0 e `prisma/staged/r-b/` apagada (objetos por fatia em `prisma/sql/objetos-r-b.sql`; o ensaio lê o alvo do `schema.prisma` e segue valendo para o P30). Régua 38/38.

## 4. O que FALTA (na ordem)

**P11a, P11b e P13a FECHADOS em 25/09.** O P13a foi integrado (`merge --no-ff` `89a7ccd9` + `3361dd9a`: `portal-identificacao` na régua com banco, catraca do tipo 135 → 126); worktree, branch `impl/p13a` e banco `ankaa_implemento_p13a` apagados. **P12 FECHADO** (P12.1 + P12.2 + P12.3, ver §3).

**P14, P13b e a integração do par 2 FECHADOS em 25/09 (na nuvem)** — ver §3. Worktree `impl/p13b` e banco `ankaa_implemento_p13b` apagados depois da integração (a branch `impl/p13b` fica local, já mergeada).

⚠️ **Como a régua rodou na nuvem (banco NOVO, sem dados reais):** `migrate deploy` NÃO replaya de banco vazio (quebra em `20260406000000` com `Item_isManualMaxQuantity_idx`), então o banco foi montado como o `tests/task-match-integration.test.ts` manda: sequências `order_number_seq`/`supplier_number_seq` + `unaccent`/`immutable_unaccent`, `prisma db push`, `prisma/sql/objetos-pos-push.sql`, `seed-notification-configs.ts`; e, para ficar fiel à produção, `_ADMISSION_DOCUMENT_FILES` recriada como na migration (A → `File`; o `db push` inverte), a tabela `BudgetLayoutTask` (fica até a M4), `_Mig0924_Triage` e uma `_prisma_migrations` com as migrations marcadas aplicadas (a M3o-b foi aplicada DE VERDADE pelo SQL dela, depois de o ensaio passar 16/16 com ela pendente). Dados mínimos: setor ADMIN + usuário, "Sergio Rodrigues (teste)" com vínculo ativo (signatário da Ankaa), 2 clientes, 1 contato, 1 tinta. `.env` blindado (Firebase vazio, SMTP/Sicredi/Elotech/NFS-e/Secullum em `127.0.0.1:9`, `DISABLE_WHATSAPP=true`, `REDIS_DB=3`, `BACKUP_PATH=./.tmp/backup`, `ARTWORK_GATE_SINCE`). O Chromium do Playwright da api (1223) não baixa na nuvem: o caminho foi apontado para o headless 1194 da imagem. **Consequência: G11 e o ensaio da R-B passaram sem dado real** (0 envelopes reais; no fim, 0 fatias pendentes). **Antes do P30, rodar na máquina local:** `migrate deploy` + ensaio no `ankaa_implemento_base` (as fatias M3o-a/M3o-b pendentes contra o acervo) e a régua inteira no `ankaa_implemento` (G11 com os 19 envelopes reais).

Achado fora do escopo (P14): `PUT /budgets/:id/status {CANCELLED}` responde "cancelado com sucesso" e NÃO cancela quando ainda há veículo ativo (`cancelForTaskCancellation` sai em silêncio) — anterior ao rework.

Próximo: **revisão da Fase B** (duas lentes sobre a combinação P06..P14) e depois a **Fase C** (P20 web base → [P21 ∥ P22] → [P23 ∥ P24] → P26). Pendências da Fase B que a Fase C herda: o gêmeo da regra de Atenção `budget.ready-to-emit` no web (P22); a tela do "Assinado fora do sistema", do "Aprovar valor em nome do cliente" e do selo da assinatura no faturamento (P22); o portal web com `orderNumber`, `emission`, artes e `APPROVE_ARTWORK` (P23); o app (P24). Fora do rework: `billing-entity` e `orcamento-faturamento-a-db` apontam para o banco `ankaa_qa_e2e`.
