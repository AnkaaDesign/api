# Rework Implemento — ESTADO (25/09/2026, noite — commit zero do P14 fechado na nuvem)
...
**Commit zero do P14 FECHADO (25/09, na nuvem).** M3o-a promovida (`20260930120300_orcamento_eixo_da_assinatura`; blocos do alvo no `schema.prisma`; `objetos-pos-push.sql` com 28 CHECKs; G36 = 10 comandos, só a M3o-b), espelhos `BUDGET_SIGNATURE_STATUS`/`BUDGET_VALUE_APPROVAL_SOURCE` com rótulos ("Assinada fora do sistema"), rename D-35 inteiro na API **e no web** (`APPROVE_VALUE`, rota `/aprovar-valor`, `canApproveValue`, `waitingOnMe.valueApproval`, `decision: "APPROVE_VALUE"`; no web: `utils/portal-capabilities.ts`, `api-client/portal.ts` com `approveBudgetValue`/`usePortalApproveBudgetValue`, `pre-aprovacao-actions.tsx`, `painel.tsx`, `orcamentos/[id].tsx`, `sections.ts`; os textos "pré-aprovado" e o destino `PRE_APPROVED` ficam para o corpo do P14/P23), `docs/PORTAL-CONTRATO.md` e `PORTAL-DO-RESPONSAVEL.md` com os nomes novos. A régua pegou um furo do WIP: o G10 acusava `BudgetOfflineSignature.fileId` (FK nova da M3o-a) sem linha em `INBOUND_REFERENCES` → linha com `context: null` (protegido, nunca movido); o contexto `budgetOfflineSignature` com pasta entra com o ato, no corpo do P14. **Régua da api 32/32 verde (321 s); web G0/G6/G4/G5 verdes e vitest 641/647 no Node 24 (os 6 do menu).** App: só as cópias do contrato, conferidas pelo G5 da api (sem Flutter na nuvem).

⚠️ **Como a régua rodou na nuvem (banco NOVO, sem dados reais):** `migrate deploy` NÃO replaya de banco vazio (quebra em `20260406000000` com `Item_isManualMaxQuantity_idx`), então o banco foi montado como o `tests/task-match-integration.test.ts` manda: sequências `order_number_seq`/`supplier_number_seq` + `unaccent`/`immutable_unaccent`, `prisma db push`, `prisma/sql/objetos-pos-push.sql`, `seed-notification-configs.ts`; e, para ficar fiel à produção, `_ADMISSION_DOCUMENT_FILES` recriada como na migration (A → `File`; o `db push` inverte), a tabela `BudgetLayoutTask` (fica até a M4), `_Mig0924_Triage` e uma `_prisma_migrations` com as 271 migrations marcadas aplicadas. Dados mínimos: setor ADMIN + usuário, 2 clientes, 1 contato, 1 tinta. `.env` blindado (Firebase vazio, SMTP/Sicredi/Elotech/NFS-e/Secullum em `127.0.0.1:9`, `DISABLE_WHATSAPP=true`, `REDIS_DB=3`, `BACKUP_PATH=./.tmp/backup`, `ARTWORK_GATE_SINCE`). **Consequência: G11 e o ensaio da R-B passaram sem dado real** (0 envelopes, 0 orçamentos): provam que a M3o-b aplica e as invariantes rodam, não que os dados de produção migram. Rodar os dois de novo na máquina local, no `ankaa_implemento`/`ankaa_implemento_base`, antes da integração do par 2.

Próximo: **[P14 ∥ P13b]** — P14 (máquina do orçamento, §2A, com o commit zero M3o-a e a M3o-b no fim) no checkout principal; P13b (portal: `APPROVE_ARTWORK`, aprovar/reprovar a arte pelo `ImplementLayoutService.approveFromPortal/reproveFromPortal`, a leitura das artes pendentes) em worktree `impl/p13b` com banco `ankaa_implemento_p13b`, criado DEPOIS do commit zero do P14. Fora do rework: `billing-entity` e `orcamento-faturamento-a-db` apontam para o banco `ankaa_qa_e2e`, que está sem esquema neste ambiente (não rodaram).
