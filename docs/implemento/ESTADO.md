...
**EM CURSO — commit zero do P14 (PARCIAL, commitado como WIP em 25/09 para continuar na nuvem):** feito: M3o-a promovida (`20260930120300_orcamento_eixo_da_assinatura`, aplicada no `ankaa_implemento`; blocos do alvo no `schema.prisma`; `objetos-pos-push.sql` com 28 CHECKs; G36 = 10 comandos, só a M3o-b), espelhos `BUDGET_SIGNATURE_STATUS`/`BUDGET_VALUE_APPROVAL_SOURCE` com rótulos ("Assinada fora do sistema"), rename D-35 na API (`APPROVE_VALUE`, rota `/aprovar-valor`, `canApproveValue`, `waitingOnMe.valueApproval`; os textos "pré-aprovado" e o destino `PRE_APPROVED` ficam para o corpo do P14), contratos regerados. `tsc` 0; `portal-decisao` e `portal-recorte` verdes. **Falta no commit zero:** (1) o lado web do D-35 (`web/src/api-client/portal.ts` chama `/pre-aprovar` → 404; `utils/portal-capabilities.ts`, `components/cliente/orcamento/pre-aprovacao-actions.tsx`, `pages/cliente/painel.tsx`, `pages/cliente/orcamentos/[id].tsx`, `sections.ts`); (2) a régua inteira (não rodou depois destas mudanças); (3) commit "commit zero" limpo. Só então o worktree do P13b nasce.
