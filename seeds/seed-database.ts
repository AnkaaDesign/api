...
function parseMongoDateWithTime(dateStr: string | null, hours: number = 7, minutes: number = 30): Date | null {
  if (!dateStr) return null;
  const date = new Date(dateStr);
...
// Helper function to convert hex to HSL for natural color sorting
function hexToHSL(hex: string): { h: number; s: number; l: number } {
  ...
  return {
    h: h * 360, // Hue in degrees (0-360)
    s: s * 100, // Saturation in percentage (0-100)
    l: l * 100, // Lightness in percentage (0-100)
  };
}
...
async function migratePaints() {
  ...
  // Pre-process all colors with their HSL values for sorting
  const colorsWithHSL = colors.map((color) => {
    const hex = cleanValue(color.hex) || '#000000';
    const hsl = hexToHSL(hex);
    return { ...color, hex, ...hsl };
  });

  // Sort colors by HSL for natural color progression
  // Primary sort: Hue (rainbow order)
  // Secondary sort: Saturation (vibrant to muted)
  // Tertiary sort: Lightness (dark to light)
  colorsWithHSL.sort((a, b) => {
    // Sort by hue first (creates rainbow effect)
    if (Math.abs(a.h - b.h) > 1) {
      return a.h - b.h;
    }
    // Then by saturation (vibrant colors first)
    if (Math.abs(a.s - b.s) > 1) {
      return b.s - a.s;
    }
  ...
  console.log(`  🎨 Colors sorted by HSL (Hue → Saturation → Lightness)`);
  ...
  // Process sorted colors with index-based colorOrder
  for (let i = 0; i < colorsWithHSL.length; i++) {
    const color = colorsWithHSL[i];
    try {
      ...
      // Use index as colorOrder for HSL-sorted colors
      const colorOrder = i;
      ...
      const paint = await prisma.paint.create({
        data: {
          ...
          colorOrder, // HSL-sorted order (rainbow progression)
        },
      });
...
async function migrateTasks() {
  ...
  // Find Sergio Rodrigues user for service order creation
  const sergioUser = await prisma.user.findFirst({
    where: {
      name: { contains: 'Sergio', mode: 'insensitive' },
    },
  ...
  if (!sergioUser) {
    console.error('  ❌ Sergio user not found. Cannot create service orders.');
    throw new Error('Sergio user required for service order creation');
  }

  console.log(`  ✓ Using Sergio user: ${sergioUser.name} (${sergioUser.id}) for service orders`);
  ...
  for (const work of works) {
    try {
      ...
      } else if (work.status === 'Aguardando' || work.status === 'Aguardando Produção') {
        status = TASK_STATUS.WAITING_PRODUCTION;
        statusOrder = TASK_STATUS_ORDER[TASK_STATUS.WAITING_PRODUCTION];
      } else if (work.status === 'Em Produção' || work.started_at) {
      ...
      let commissionStatus = COMMISSION_STATUS.FULL_COMMISSION; // Default to full commission
      ...
      if (commissionField !== undefined && commissionField !== null && commissionField !== '') {
        ...
        if (commissionValue === -1) {
          commissionStatus = COMMISSION_STATUS.SUSPENDED_COMMISSION;
        } else if (commissionValue === 0) {
          commissionStatus = COMMISSION_STATUS.NO_COMMISSION;
        } else if (commissionValue === 0.5) {
          commissionStatus = COMMISSION_STATUS.PARTIAL_COMMISSION;
        } else if (commissionValue === 1) {
          commissionStatus = COMMISSION_STATUS.FULL_COMMISSION;
        } else {
        ...
        if (status === TASK_STATUS.COMPLETED) {
          ...
          commissionStatus = COMMISSION_STATUS.FULL_COMMISSION;
        } else if (status === TASK_STATUS.PREPARATION || status === TASK_STATUS.WAITING_PRODUCTION) {
          // Preparation/waiting tasks don't have commission yet (will get it when started)
          commissionStatus = COMMISSION_STATUS.NO_COMMISSION;
        } else {
      ...
      // Parse dates with term/finishedAt sync
      const term = parseMongoDate(work.term);
      const finishedAt = parseMongoDate(work.finished_at);
      ...
      const taskData: any = {
        ...
        entryDate: entryDate,
        forecastDate: entryDate, // Set forecastDate = entryDate as required
        createdAt: entryDate || (() => { const d = new Date(); d.setHours(7, 30, 0, 0); return d; })(),
        ...
        try {
          const observation = await prisma.observation.create({
            data: {
              ...
              createdAt: parseMongoDateWithTime(work.entry_date) || (() => { const d = new Date(); d.setHours(7, 30, 0, 0); return d; })(),
              updatedAt: parseMongoDate(work.updatedAt) || new Date(),
            },
          });
      ...
      // Create 4 standard service orders for ALL tasks
      const serviceOrderTypes = [
        { type: SERVICE_ORDER_TYPE.COMMERCIAL, description: 'Enviar Orçamento' },
        { type: SERVICE_ORDER_TYPE.LOGISTIC, description: 'Configurar Tarefa' },
        { type: SERVICE_ORDER_TYPE.FINANCIAL, description: 'Enviar Boleto' },
        { type: SERVICE_ORDER_TYPE.ARTWORK, description: 'Elaborar Layout' },
      ] as const;

      for (const soConfig of serviceOrderTypes) {
        try {
          // Determine service order status based on task status
          let serviceOrderStatus = SERVICE_ORDER_STATUS.PENDING;
          let statusOrder = SERVICE_ORDER_STATUS_ORDER[SERVICE_ORDER_STATUS.PENDING];
          let startedAt = null;
          let finishedAt = null;
          let startedById = null;
          let completedById = null;

          if (status === TASK_STATUS.WAITING_PRODUCTION) {
            // Task is waiting production -> service orders are PENDING
            serviceOrderStatus = SERVICE_ORDER_STATUS.PENDING;
            statusOrder = SERVICE_ORDER_STATUS_ORDER[SERVICE_ORDER_STATUS.PENDING];
          } else if (status === TASK_STATUS.COMPLETED) {
            // Task is completed -> service orders are COMPLETED
            serviceOrderStatus = SERVICE_ORDER_STATUS.COMPLETED;
            statusOrder = SERVICE_ORDER_STATUS_ORDER[SERVICE_ORDER_STATUS.COMPLETED];
            startedAt = task.startedAt || task.entryDate || new Date();
            finishedAt = task.finishedAt || new Date();
            startedById = sergioUser.id;
            completedById = sergioUser.id;
          }
          ...
          await prisma.serviceOrder.create({
            data: {
              ...
              description: soConfig.description,
              taskId: task.id,
              createdById: sergioUser.id,
              startedAt,
              finishedAt,
              startedById,
              completedById,
          ...
          console.error(`    ❌ Failed to create ${soConfig.type} service order:`, error);
        }
      }

      // Create PRODUCTION service orders from CSV (the old service orders)
      let hasProductionServiceOrders = false;
      ...
      for (let i = 0; i < 8; i++) {
        ...
        if (serviceName && serviceName.trim() !== '') {
          hasProductionServiceOrders = true;

          try {
            // First, ensure the service exists in the Service catalog
            ...
            // Determine status: if task is COMPLETED, all PRODUCTION service orders should be COMPLETED
            const isServiceCompleted = status === TASK_STATUS.COMPLETED || serviceDone === 'true' || serviceDone === true;

            // Create the PRODUCTION service order from CSV
            await prisma.serviceOrder.create({
              data: {
                type: SERVICE_ORDER_TYPE.PRODUCTION,
                taskId: task.id,
                description: serviceName,
                status: isServiceCompleted
                  ? SERVICE_ORDER_STATUS.COMPLETED
                  : SERVICE_ORDER_STATUS.PENDING,
                statusOrder: isServiceCompleted
                  ? SERVICE_ORDER_STATUS_ORDER[SERVICE_ORDER_STATUS.COMPLETED]
                  : SERVICE_ORDER_STATUS_ORDER[SERVICE_ORDER_STATUS.PENDING],
                createdById: sergioUser.id,
                startedAt: isServiceCompleted
                  ? task.startedAt || task.entryDate || new Date()
                  : null,
                finishedAt: isServiceCompleted ? task.finishedAt || new Date() : null,
                startedById: isServiceCompleted ? sergioUser.id : null,
                completedById: isServiceCompleted ? sergioUser.id : null,
              },
            });
          } catch (error) {
            console.error(`    ❌ Failed to create PRODUCTION service order ${serviceName}:`, error);
          }
        }
      }

      // If task has NO PRODUCTION service orders from CSV, create default "Logomarca Padrão"
      if (!hasProductionServiceOrders) {
        try {
          // Ensure "Logomarca Padrão" exists in Service catalog
          let defaultServiceId = serviceMap.get('Logomarca Padrão');
          if (!defaultServiceId) {
            let defaultService = await prisma.service.findFirst({
              where: { description: 'Logomarca Padrão' },
            });

            if (!defaultService) {
              defaultService = await prisma.service.create({
                data: { description: 'Logomarca Padrão' },
              });
              console.log(`    📝 Created service: Logomarca Padrão`);
            }

            defaultServiceId = defaultService!.id;
            serviceMap.set('Logomarca Padrão', defaultServiceId);
          }

          // Create default PRODUCTION service order
          const isCompleted = status === TASK_STATUS.COMPLETED;
          await prisma.serviceOrder.create({
            data: {
              type: SERVICE_ORDER_TYPE.PRODUCTION,
              taskId: task.id,
              description: 'Logomarca Padrão',
              status: isCompleted
                ? SERVICE_ORDER_STATUS.COMPLETED
                : SERVICE_ORDER_STATUS.PENDING,
              statusOrder: isCompleted
                ? SERVICE_ORDER_STATUS_ORDER[SERVICE_ORDER_STATUS.COMPLETED]
                : SERVICE_ORDER_STATUS_ORDER[SERVICE_ORDER_STATUS.PENDING],
              createdById: sergioUser.id,
              startedAt: isCompleted ? task.startedAt || task.entryDate || new Date() : null,
              finishedAt: isCompleted ? task.finishedAt || new Date() : null,
              startedById: isCompleted ? sergioUser.id : null,
              completedById: isCompleted ? sergioUser.id : null,
            },
          });
        } catch (error) {
          console.error(`    ❌ Failed to create default PRODUCTION service order:`, error);
        }
      }
        ...
        try {
          ...
          const bonus = await prisma.bonus.create({
            data: {
              ...
              weightedTasks: weightedTaskCount, // Weighted task count for bonus calculation
...
async function seedTaxTables2025() {
  console.log('\n💰 Seeding 2025 Tax Tables (INSS & IRRF)...');
  ...
  try {
    ...
    console.log('  ✅ INSS 2025 table seeded with 4 brackets');
    console.log('  ✅ IRRF 2025 table seeded with 5 brackets');
  } catch (error: any) {
...
async function seedNotificationPreferences() {
  console.log('\n🔔 Seeding Notification Preferences for all users...');
  ...
  try {
    ...
    const defaultPreferences = [
      ...
      { type: 'TASK', eventType: 'status', channels: ['IN_APP', 'EMAIL', 'MOBILE_PUSH'] as NotificationChannel[], mandatory: true },
      { type: 'TASK', eventType: 'deadline', channels: ['IN_APP', 'EMAIL', 'MOBILE_PUSH', 'WHATSAPP'] as NotificationChannel[], mandatory: true },
      { type: 'TASK', eventType: 'assignment', channels: ['IN_APP', 'EMAIL', 'MOBILE_PUSH'] as NotificationChannel[], mandatory: true },
      { type: 'TASK', eventType: 'artwork', channels: ['IN_APP', 'EMAIL'] as NotificationChannel[], mandatory: true },
      { type: 'TASK', eventType: 'budget', channels: ['IN_APP', 'EMAIL'] as NotificationChannel[], mandatory: true },
      { type: 'TASK', eventType: 'invoice', channels: ['IN_APP', 'EMAIL'] as NotificationChannel[], mandatory: true },
      { type: 'TASK', eventType: 'term', channels: ['IN_APP', 'EMAIL', 'MOBILE_PUSH'] as NotificationChannel[], mandatory: true },
      { type: 'TASK', eventType: 'forecast', channels: ['IN_APP', 'EMAIL'] as NotificationChannel[], mandatory: true },

      // Optional order notifications
      { type: 'ORDER', eventType: 'created', channels: ['IN_APP'] as NotificationChannel[], mandatory: false },
      { type: 'ORDER', eventType: 'status', channels: ['IN_APP', 'EMAIL'] as NotificationChannel[], mandatory: false },
      { type: 'ORDER', eventType: 'overdue', channels: ['IN_APP', 'EMAIL', 'WHATSAPP'] as NotificationChannel[], mandatory: false },

      // Optional stock notifications
      { type: 'STOCK', eventType: 'low', channels: ['IN_APP', 'EMAIL'] as NotificationChannel[], mandatory: false },
      { type: 'STOCK', eventType: 'out', channels: ['IN_APP', 'EMAIL', 'WHATSAPP'] as NotificationChannel[], mandatory: false },
    ];
    ...
    console.log(`  ✅ Seeded preferences for ${users.length} users (${prefCount} preferences total)`);
  } catch (error: any) {
    console.error('  ❌ Error seeding notification preferences:', error.message);
    throw error;
  }
}

// ============================================================================
// WELCOME MESSAGES BY SECTOR
// ============================================================================
...
function createSimpleWelcomeMessage(privilege: SECTOR_PRIVILEGES) {
  const blocks: any[] = [];
  ...
  blocks.push(heading(3, [text(`Bem-vindo ao Sistema Ankaa Design`)], { fontSize: 'xl', fontWeight: 'semibold' }));

  blocks.push(paragraph([bold('Você faz parte da equipe Ankaa Design!')]));

  blocks.push(
    paragraph([
      text(
        'Este é o sistema de gestão interno da empresa. Aqui você pode acompanhar tarefas, clientes, estoque, produção e muito mais.',
      ),
    ]),
  );
  ...
  blocks.push(list(true, ['O que você estava fazendo', 'O que aconteceu', 'O que você esperava']));
...
async function seedWelcomeMessages() {
  console.log('\n📨 Seeding Welcome Messages by sector...');
  ...
  try {
    ...
    if (!adminUser) {
      console.log('  ⚠️  No admin user found, skipping welcome messages');
      return;
    }

    // Delete existing welcome messages
    ...
    const sectorsByPrivilege = new Map<SECTOR_PRIVILEGES, { id: string; name: string }[]>();
    for (const sector of sectors) {
    ...
    for (const [privilege, privilegeSectors] of sectorsByPrivilege) {
      ...
      await prisma.message.create({
        data: {
          title: `Bem-vindo ao Sistema Ankaa Design`,
          content: createSimpleWelcomeMessage(privilege),
          status: 'ACTIVE',
      ...
      console.log(`  ✅ Created message for ${privilege}: ${sectorNames} (${users.length} users)`);
      createdCount++;
    }

    console.log(`  ✅ Created ${createdCount} welcome messages total`);
  } catch (error: any) {
    console.error('  ❌ Error seeding welcome messages:', error.message);
    // Don't throw - welcome messages are not critical
  }
}

// ============================================================================
// UPDATE POSITION REMUNERATIONS TO 2025 ACTUAL VALUES (OPTIONAL)
// ============================================================================
async function updatePositionRemunerationsTo2025() {
  console.log('\n💰 Updating Position Remunerations to 2025 Actual Values...');

  // Actual salary values from payroll PDFs (August-October 2025)
  const positionRemunerations = [
  ...
  for (const posRem of positionRemunerations) {
    try {
      ...
      console.log(`  ✅ Updated ${position.name}: R$ ${currentRemuneration.toFixed(2)} → R$ ${posRem.currentValue.toFixed(2)}`);
      updated++;
    } catch (error: any) {
      console.error(`  ❌ Error updating ${posRem.positionName}:`, error.message);
    }
  }

  console.log(`  ✅ Updated ${updated} positions, ${skipped} already correct, ${notFound} not found`);
}

// Configuration flag for optional features
const ENABLE_2025_SALARY_UPDATE = false; // Set to true to update positions to 2025 actual values
...
async function main() {
  ...
  try {
    ...
    const deletions = [
      ...
      {
        name: 'MonetaryValue (Positions)',
        fn: () => prisma.monetaryValue.deleteMany({ where: { positionId: { not: null } } }),
      },
    ...
    await migrateUsers();
    await seedNotificationPreferences(); // Seed notification preferences after users are created
    ...
    await seedTaxTables2025(); // Seed tax tables after payrolls
    await seedWelcomeMessages(); // Seed welcome messages at the end

    // Optional: Update position salaries to 2025 actual values
    if (ENABLE_2025_SALARY_UPDATE) {
      await updatePositionRemunerationsTo2025();
    }
    ...
    try {
      ...
      const additionalCounts = await Promise.all([
        prisma.taxTable.count(),
        prisma.taxBracket.count(),
        prisma.userNotificationPreference.count(),
        prisma.message.count(),
      ]);

      console.log(`  - Tax Tables: ${additionalCounts[0]}`);
      console.log(`  - Tax Brackets: ${additionalCounts[1]}`);
      console.log(`  - Notification Preferences: ${additionalCounts[2]}`);
      console.log(`  - Welcome Messages: ${additionalCounts[3]}`);
    } catch (summaryError) {
      console.log(
